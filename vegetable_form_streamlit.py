...
st.markdown("""
    ...
    .doc-id { border: 1.5px solid #000; padding: 4px 12px; font-weight: bold; font-size: 16px; }
...
st.markdown(f"""
    <div class="header-container">
        ...
        <div style="text-align: right;">
            ...
            <div style="font-size: 13px;">มีผลใช้งาน: 2026-05-08</div>
        </div>
    </div>
...
# --- 4. ส่วนที่ 1 — ข้อมูลผู้ส่งมอบ (สรุปสั้นๆ) ---
...
with c3:
    ...
    country_main = st.selectbox("ประเทศแหล่งปลูก (หลัก)", ["ไทย", "จีน", "อื่นๆ"], key="c_main")
...
for i, item in enumerate(st.session_state.items):
    with st.container():
        ...
        r4c3.selectbox("ประเทศแหล่งปลูก", ["ไทย", "จีน", "อื่นๆ"], key=f"country_{i}")

        # ส่วนที่อยู่ (Dropdown)
        st.markdown("📍 **ที่อยู่แหล่งปลูก**")
        r5c1, r5c2, r5c3 = st.columns(3)
        r5c1.selectbox("จังหวัด/มณฑล", provinces, key=f"prov_{i}")
