...
# ฟังก์ชันแปลงรูปภาพเป็น Base64 เพื่อแสดงในหัวข้อหน้าเว็บ
...
# --- 2. ส่วนหัวแบบฟอร์ม (Header) ตามรูปภาพที่กำหนด ---
logo_path = "image_9482bc.png" #
logo_base64 = get_image_base64(logo_path)

# สร้าง Header ด้วย HTML/CSS ให้เหมือน image_9747d4.png
header_html = f"""
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #2e7d32; padding-bottom: 10px; margin-bottom: 20px;">
        <div>
            {f'<img src="data:image/png;base64,{logo_base64}" width="160">' if logo_base64 else '<b>[CPRAM LOGO]</b>'}
        </div>
...
# --- 3. ส่วนที่ 1 — ข้อมูลผู้ส่งมอบ (ลบช่อง Email และผู้กรอกออก) ---
...
# --- 4. ส่วนที่ 2 — รายการวัตถุดิบ (ดึงฟิลด์กลับมาครบตาม PDF) ---
...
for i in range(st.session_state.items_count):
    with st.container():
        ...
        # รายละเอียดการปลูก [cite: 10, 11, 12]
        ...
        # ที่อยู่ (ดึงข้อมูลจังหวัด/อำเภอ/ตำบล)
...
# --- 5. การสร้าง PDF (ล็อกฟอนต์ Sarabun เพื่อรองรับภาษาไทย) ---
st.write("---")
if st.button("✅ ยืนยันข้อมูลและดาวน์โหลด PDF", type="primary", use_container_width=True):
    if not s_name:
        ...
        pdf = FPDF()
        pdf.add_page()
        # ใช้ Sarabun-Regular.ttf จากในระบบของคุณ
        font_path = "Sarabun-Regular.ttf" 
        if os.path.exists(font_path):
            pdf.add_font('Sarabun', '', font_path, uni=True)
            pdf.set_font('Sarabun', '', 14)
            
            pdf.cell(0, 10, f"แบบฟอร์มผู้ส่งมอบ: {s_name}", ln=True, align='C')
            pdf.cell(0, 10, f"วันที่: {s_date}", ln=True)
            
            for i in range(st.session_state.items_count):
                pdf.ln(5)
                pdf.cell(0, 10, f"รายการที่ {i+1}: {st.session_state.get(f'mat_{i}')} | {st.session_state.get(f'qty_{i}')} KG", ln=True)
            
            # ป้องกัน Error ภาษาไทยด้วย latin-1 replace
            pdf_data = pdf.output(dest='S').encode('latin-1', 'replace')
            st.download_button("📥 ดาวน์โหลดไฟล์ PDF", data=pdf_data, file_name=f"Form_{s_name}.pdf")
        else:
